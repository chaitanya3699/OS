	...
	for (int i=0; i < op_cnt; i++) {
		switch (op[i]) {
			...
			case OP_MAPREAD: {
				...
				// TODO
				char *mem;
				mem = (char *) mmap(NULL, MYDEV_LEN, PROT_READ, mmap_flags, dev_fd, 0);

				if(mem == MAP_FAILED){  	//if mmap failes 
				...
				int flag = 0;
				if(msg_len!=0){						//if length of msg is '0', then no need to compare
					int i=0;
					while(i<MYDEV_LEN){
						//comparing string with entire device mem
						if(*(mem+i) != msg[i%msg_len]){	 //if they are not equal
							flag = 1;
							ret = EXIT_FAILURE;			 //then setting return value to 'EXIT_FAILURE'
						}
						i++;
					}
				}
				else{								//if length of msg is '0', then
					int i=0;
					while(i<MYDEV_LEN){
						char ch = *(mem+i);      //reading the data.
						i++;
					}
				}
				//cout << flag << " " << msg_len << endl;
			...
			case OP_MAPWRITE: {
				...
				// TODO
				char *mem;
				mem = (char *) mmap(NULL, MYDEV_LEN, PROT_WRITE, mmap_flags, dev_fd, 0);

				if(mem == MAP_FAILED){  	//if mmap failes 
					exit(EXIT_FAILURE);		//error
				}
				// Write the message to devicemem from msg.
				// DONOT define an array of MYDEV_LE// --message is not required with read alone
	// --message is must along with writeN. Seriously! Dont need array of MYDEV_LEN size.
				// TODO. Hint use loop & modulus operator on msg to copy the string to entire device_mem
				int i=0;
				while(i<MYDEV_LEN){
					for(int j=i; j<i+msg_len && j<MYDEV_LEN; j++){	//copying string to entire device memory
						*(mem+j) = msg[j%msg_len];
					}
					i += msg_len;
				}
